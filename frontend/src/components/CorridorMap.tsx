'use client'
/**
 * CorridorMap.tsx — v4.2 Cinematic Rail
 * FIX: mapRef.current null error — corridors now wait for map to be ready
 */
...
export default function CorridorMap({ corridors, selectedId, onSelect, heatmapData = [] }: Props) {
  ...
  const mapReadyRef  = useRef<boolean>(false)  // NEW: track when map is truly ready
  ...
  useEffect(() => {
    ...
    import('leaflet').then(L => {
      ...
      mapRef.current  = map
      mapReadyRef.current = true  // FIX: mark map as ready AFTER it's assigned

      // Apply Financial Rail DNA tint
      const pane = map.getPane('tilePane') as HTMLElement | undefined
      if (pane) {
        pane.style.filter = 'hue-rotate(200deg) saturate(0.7) brightness(0.75)'
      }

      // FIX: draw corridors immediately after map is ready if they're already loaded
  ...
  // ── Draw corridors whenever data changes ───────────────────
  function drawCorridors(L: any, map: any) {
    // Safety guard — never call addTo on null
    if (!map) return
    ...
    corridors.forEach(c => {
      ...
      const line = L.polyline(arcPts, {
        ...
        smoothFactor: 1,
      }).addTo(map)  // use local `map` param, never mapRef.current directly
      ...
      const tip = `
        <div style="min-width:200px;background:#0b1628;border:1px solid #1a2d4a;
          ...
          <div style="margin-top:8px;display:grid;grid-template-columns:1fr 1fr;
            ...
            <span style="color:#94A3B8">Heat Level</span>
  ...
  // ── Redraw when corridors/selection changes ────────────────
  useEffect(() => {
    if (!mapReadyRef.current || !mapRef.current || corridors.length === 0) return
    import('leaflet').then(L => {
      if (!mapRef.current) return  // double-check after async
      drawCorridors(L, mapRef.current)
    })
  }, [corridors, selectedId, heatmapData]) // eslint-disable-line
